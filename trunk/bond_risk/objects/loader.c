...
extern bool currency_curve_need_refresh( int id );

// Provided by the user.
extern void* calculate_bond( void* id );
extern void* calculate_currency_curve( void* id );
...
void load_currency_curve( int id );
...
void load_bonds()
{
    ...
    //    ((bond*)obj_loc[id])->name = 0;
        ...
        ((bond*)obj_loc[id])->currency_curve = 2;
...
void load_currency_curve()
{
    ...
    while (( row = mysql_fetch_row(result) )) {
        ...
        obj_loc[id] = new currency_curve;

        ((currency_curve*)obj_loc[id])->last_published = 0;
        ((currency_curve*)obj_loc[id])->status = STOPPED;
        ((currency_curve*)obj_loc[id])->calculator_fpointer = &calculate_currency_curve;
        ((currency_curve*)obj_loc[id])->need_refresh_fpointer = &currency_curve_need_refresh;
        ((currency_curve*)obj_loc[id])->type = 0;
    //    ((currency_curve*)obj_loc[id])->name = 0;

        ((currency_curve*)obj_loc[id])->interest_rate_feed = 1;

        cout << "New currency_curve created" << endl;
