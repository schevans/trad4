...
Trad4 is a declarative language where you describe types (mapped to C structures), define the relationship between the types (in a db), and define what the objects do when they fire.
...
There are three applications discussed in this document: hello_world, simple_maths and bond_risk. There are other applications available and written up online.
...
It's useful for locking down the numbers while you make changes. It's specfic to data sets so it's only really useful in development, but it's also used in test_app as there's only one data set.
